...
ALL_GENS = glob.glob(f"../data_for_paper_comparison/{DATASET}/Test/*.pkl")
...
for gen_id, genpath in enumerate(ALL_GENS):
    ...
    entity = os.path.basename(genpath).replace("_data.pkl", "")
    print(f"Doing {entity}")

    for sent_id, sent in enumerate(curr_sentences):
        json_line = {
            "custom_id": f"req_{gen_id}_{sent_id}_{entity}",
            "method": "POST",
...
with open(f'big_concept_batch_{DATASET}.jsonl', 'w', encoding='utf-8') as file:
